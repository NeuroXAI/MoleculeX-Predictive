...
\AR{We have strengthened the MOSES caveat with a boldface ``Important caveat'' statement explicitly noting that all pIC50 predictions on MOSES are model-generated outputs that cannot be validated against ground-truth biological activity and should be interpreted as demonstrating chemical plausibility, not experimental validity. We have also clarified that the model was trained and initially tested on the ChEMBL dataset, which contains ground-truth experimental pIC50 values, so the reported cross-validation metrics ($R^2$, MAE, RMSE) are validated against experimental data; only the MOSES evaluation lacks ground-truth pIC50 for validation.}
...
\AR{The MOSES caveat has been strengthened with a boldface warning. A new ``Out-of-distribution generalization'' paragraph explicitly acknowledges that generalization to structurally distant chemical series has not been evaluated and predictions on such compounds should be treated with caution. We note that scaffold-based cross-validation metrics reflect within-domain generalization only. We have also clarified that the model was trained and initially tested on the ChEMBL dataset, which contains ground-truth experimental pIC50 values, so the reported performance metrics are validated against experimental data; only the MOSES evaluation lacks ground-truth pIC50 for validation.}
